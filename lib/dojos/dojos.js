...
exports.register = function (server, options, next) {
  ...
  function handleGet (cmd) {
    return function (request, reply) {
      reply.act({cmd: cmd, role: 'cd-dojos'}, {locality: server.methods.locality(request)});
    };
  }

  function handlePost (cmd) {
    return function (request, reply) {
      var msg = _.extend(request.payload, {cmd: cmd, role: 'cd-dojos'}, {locality: server.methods.locality(request)});
      reply.act(msg);
    };
  }
  ...
  function handlePostWithUser (cmd) {
    return function (request, reply) {
      ...
      server.methods.validateLogin(request.seneca, token, function (err, user, code) {
        ...
        var msg = _.extend(request.payload, {cmd: cmd, role: 'cd-dojos'}, user, {locality: server.methods.locality(request)});
        reply.act(msg);
      });
    };
  }
